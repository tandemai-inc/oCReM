...
def __fragment_mol_link(
    ...
    frags_1 = rdMMPA.FragmentMol(
        mol1, pattern="[#1]!@!=!#[!#1]", maxCuts=1, resultsAsMols=True, maxCutBonds=100
    )
    frags_2 = rdMMPA.FragmentMol(
        mol2, pattern="[#1]!@!=!#[!#1]", maxCuts=1, resultsAsMols=True, maxCutBonds=100
    )
...
def __frag_replace(
    mol1, mol2, frag_sma, replace_sma, radius, frag_ids_1=None, frag_ids_2=None
):
    """
    INPUT
...
def __gen_replacements(
    ...
    **kwargs,
    ...
    replacements = set()  # to store unused   row_id: (frag_sma, core, ids)
    returned_values = 0
    ...
    for env, core, *ids in f:
        # if link = True ids is two tuples, if link = False ids is a single tuple
        ...
        if (
            ...
            # if filter_func:
            #     row_ids = set(filter_func(row_ids, db_manager, radius))

            if max_replacements is not None:
                n = min(len(results), preliminary_return)
    ...
    if max_replacements is not None:
        ...
        #     )
        # else:
        selected_results = random.sample(list(replacements), n)
        for core_smi, core_sma, freq, frag_sma, core, ids in selected_results:
            if core_smi != core:
                if return_frag_smi_only:
                    yield core_smi
                else:
...
def grow_mol(
    ...
    **kwargs,
    ...
    if replace_ids:
        ...
        ids = set(a.GetIdx() for a in m.GetAtoms() if a.GetAtomicNum() == 1).difference(
            ids
    ...
    if ncores == 1:
        ...
        try:
            for items in p.imap(
                ...
                for smi, m, rxn, freq in items:
                    if max_replacements is None or (
                        max_replacements is not None
                        and len(products) < max_replacements
                    ):
                        if smi not in products:
                            products.add(smi)
                            res = [smi]
    ...
    **kwargs,
    ...
    if replace_ids_1:
        ...
        protected_ids_1 = set(protected_ids_1) | set(
            range(mol1.GetNumAtoms())
        ).difference(replace_ids_1)
    if isinstance(mol2, Chem.Mol):
        ...
        if replace_ids_2:
            ...
            protected_ids_2 = set(protected_ids_2) | set(
                range(mol2.GetNumAtoms())
            ).difference(replace_ids_2)
    else:
        protected_ids_2 = None
